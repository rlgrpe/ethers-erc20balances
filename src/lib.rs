...
pub trait Erc20BalancesMiddleware {
    ...
    async fn get_erc20_balances_at_block(
        &self,
        address: Vec<Address>,
        token_addresses: Vec<Address>,
        block: BlockId,
    ) -> Result<HashMap<Address, HashMap<Address, U256>>, Self::Error>;
    async fn get_erc20_balances_at_block_with_chain_id(
        &self,
        address: Vec<Address>,
        token_addresses: Vec<Address>,
        chain_id: U256,
    ...
    Self: Clone,
{
    ...
    async fn get_erc20_balances_at_block(
        ...
        let chain_id = self.get_chainid().await?;
        self.get_erc20_balances_at_block_with_chain_id(address, token_addresses, chain_id, block)
            .await
    }
    ...
    async fn get_erc20_balances_at_block_with_chain_id(
        ...
        let contract = BalanceChecker::new(contract_address, Arc::new(self.clone()));
        let balances = contract.balances(address.clone(), token_addresses.clone()).block(block).call().await.map_err(|e|
            ProviderError::CustomError(format!("Failed to get balances for addresses {:?} and tokens {:?} on chain id {}; {:?}", address, token_addresses, chain_id, e))
        )?;
